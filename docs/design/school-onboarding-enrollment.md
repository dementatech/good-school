...
5. **Classes/streams and subjects** — seed from a platform-wide template for the chosen curriculum(s), then let the school customize (add streams, drop/add elective subjects). For A-Level, present the national combination catalog as a toggle-list (grouped Science/Arts/Business, subsidiary rules pre-filled) rather than a build-your-own-combination form — see `subject-selection-module.md` §3.2b.
